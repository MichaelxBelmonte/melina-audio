...
native runtimes. Windows ARM64 uses a source-vendored tract-linalg patch that routes its generated
GNU ARM64 assembly through LLVM Clang while preserving the MSVC ABI; the exact upstream archive,
checksum, and patch are recorded beside the vendored source. `packageAppImage` uses `jpackage` to
include Java; users do not need to install a JDK. Desktop-only builds pass
`-Pmelina.desktopOnly`, so no Android SDK is required or configured.
